...
def _(mo):
    mo.md(r"""
    # What is the lowest (3-sequence) entropy you can get that still gives you 50% reward rate

    - balanced de Bruijn sequence?
    """)
    return
...
@app.cell
def _(MatchingPennies2, np, plot_subject_and_computer_choice, plt, splstyle):
    def test_specific_sequence(sequence=[0, 0, 1], num_trials=600):
    ...
    _cpu_choice, _s_choice, _s_reward = test_specific_sequence(
        sequence = [
    ...
    with plt.style.context(splstyle.get_style('nature-reviews')): 
        _fig, _axs = plot_subject_and_computer_choice(
            computer_choice=_cpu_choice, subject_choice=_s_choice, subject_reward=_s_reward)
        plt.show()
    return
...
def _():
    ...
    def check_sequence(seq, N=4):
        """
        For a periodic sequence, check whether every N-gram context
        has a balanced (or absent) successor distribution.
        Returns dict of {context: {successor counts}} for inspection.
        """
        seq = list(seq)
        period = len(seq)
        # tile enough to see all wrap-around contexts
        extended = seq * 4
    
        from collections import defaultdict
        context_successors = defaultdict(list)
    
        for i in range(period * 2):  # two full periods to capture all contexts
            ctx = tuple(extended[i:i+N])
            successor = extended[i+N]
            context_successors[ctx].append(successor)
    
        return dict(context_successors)

    def is_safe_sequence(seq, N=4):
        """
        A sequence is 'safe' against an N-gram detector if every
        N-gram context that appears is followed equally by 0 and 1.
        """
        ctx_succ = check_sequence(seq, N)
        for ctx, successors in ctx_succ.items():
            counts = [successors.count(0), successors.count(1)]
            # must be balanced (equal counts of 0 and 1)
            if counts[0] != counts[1]:
                return False, ctx, counts
        return True, None, None

    # Search over short periods
    for period in range(5, 16):
        for seq in product([0, 1], repeat=period):
            safe, bad_ctx, counts = is_safe_sequence(seq, N=4)
            if safe:
                print(f"Period {period}: {list(seq)}")
                break
    return
